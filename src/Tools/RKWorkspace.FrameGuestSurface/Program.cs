...
Console.WriteLine($"Pages: {result.DocumentFrameState.PageCount}");
Console.WriteLine($"CurrentPage: {result.DocumentFrameState.CurrentPage}");
Console.WriteLine($"PageFrameUpdates: {result.DocumentFrameState.Updates.Count}");
Console.WriteLine($"MultiPageNavigation: {(result.MultiPageNavigationPrepared ? "OK" : "FAILED")}");
