...
public static class LivingLensApplication
{
    ...
    public static int RunSmokeTest()
    {
        ...
        try
        {
            ...
            Console.WriteLine($"LensOpen: {(session.LensOpen ? "OK" : "FAILED")}");
            Console.WriteLine($"MiniAblage: {(session.MiniAblageVisible ? "OK" : "FAILED")}");
            ...
            Console.WriteLine($"TargetGhost: {(session.TargetGhostVisible ? "OK" : "FAILED")}");
