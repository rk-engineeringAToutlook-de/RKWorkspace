...
$reportPath = Join-Path $root 'release\ma017\reports\ma017-repeatability-report.md'
$runs = [Math]::Max(1, [Math]::Min($Count, 5))
$results = New-Object System.Collections.Generic.List[string]

Write-Output 'RK Workspace MA017 Repeatability'
Write-Output '--------------------------------'
...
for ($index = 1; $index -le $runs; $index++) {
    ...
    foreach ($line in @('NoFileIngress: SUCCESS', 'Recovery: SUCCESS', 'RESULT: SUCCESS')) {
...
Write-Output 'MA017Repeatability: SUCCESS'
