...
if ([string]::IsNullOrWhiteSpace($ConfigPath)) {
    $ConfigPath = Join-Path $root 'release\ma017\config\windows-local.sample.json'
}

if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $root 'release\ma017\reports\ma017-pilot-report.md'
}
...
Write-Output 'MA017PilotReport: SUCCESS'
